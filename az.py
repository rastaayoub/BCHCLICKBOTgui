...
def Connection(phone_number,root,monitorframe,Proc_List):
 process = sub.Popen([sys.executable,'teleconnect.py', phone_number], stdout=sub.PIPE, stdin=sub.PIPE, stderr=sub.PIPE,shell=True)
 ...
 if stdout== b'Please enter the code you received: ':
   ...
   def vasy():
    recon = sub.Popen([sys.executable,'teleconnect.py', phone_number], stdout=sub.PIPE, stdin=sub.PIPE, stderr=sub.PIPE,shell=True)
